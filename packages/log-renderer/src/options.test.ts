...
interface TooltipOption {
  ...
  readonly axisPointer: { readonly axis: string; readonly type: string };
...
describe("buildScalarLogOption", () => {
  test("builds synchronized side-by-side scalar tracks", () => {
    ...
    expect(series.every((item) => item.sampling === "minmax")).toBe(true);
  ...
  test("enables linked hover and synchronized inside navigation", () => {
    ...
    expect(tooltip.axisPointer).toMatchObject({ axis: "y", type: "cross" });
    expect(
      tooltip.formatter({
        seriesId: "gr",
        seriesName: "GR",
